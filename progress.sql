...
create or replace function get_names(out int, out text, out text) returns setof record as
$$
   SELECT acc_id,lastname,firstname from account where admin_prev = false
$$
language 'sql';
...
create or replace function searchbill(in par_name text, out int, out TEXT, out text, out text) returns setof record as
  $$
    select acc_id, firstname, lastname, address from account where concat(firstname, ' ',lastname) ilike par_name
  $$
   language 'sql';
